...
**Какие таблицы под RLS.** RLS ставится ТОЛЬКО на таблицы, содержащие приватные данные компании: `audit_log`, `api_keys`, `invitations`, `projects`, `estimates`, все будущие доменные. **`memberships` и `sessions` — БЕЗ RLS**, потому что они N-к-N справочники принадлежности (юзер может быть в нескольких компаниях, `/me` показывает список всех его компаний — под контекстом одной компании RLS показал бы только строки одной). Изоляция для них обеспечивается на слое приложения: `sessions` защищены signed cookie (без валидной подписи session_id не подделаешь), `memberships` всегда читаются с явным `WHERE user_id = ctx.userId`. Никакой эндпоинт не отдаёт memberships/sessions чужих юзеров. `users` и `companies` тоже без RLS — глобальные сущности.
